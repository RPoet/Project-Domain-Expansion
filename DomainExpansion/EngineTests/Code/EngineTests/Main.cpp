...
#include "EngineTests/Framework/FrameworkReplayTest.h"
...
enum class EngineTestsExecutionCode : int32
{
	...
	replayLoadFailed = 6,
	replayCommandFailed = 7,
	replayValidationFailed = 8,
};
...
static string buildEngineTestsNarrowText(const wstring& wideText)
{
	string narrowText = {};
	narrowText.reserve(wideText.length());
	for (size_t characterIndex = 0; characterIndex < wideText.length(); ++characterIndex)
	{
		narrowText.push_back(static_cast<char>(wideText[characterIndex]));
	}
...
static EngineTestsRunOptions parseEngineTestsRunOptions(const WideStringPointer commandLine)
{
	...
	if (tryGetEngineTestsArgumentValue(commandLineText, L"-mode=", argumentValue))
	{
		...
		else if (argumentValue == L"replay")
		{
			runOptions.runMode = EngineTestsRunMode::replayMode;
		}
	}
	...
	if (tryGetEngineTestsArgumentValue(commandLineText, L"-editor_replay_file=", argumentValue))
	{
		runOptions.editorReplayFilePath = buildEngineTestsNarrowText(argumentValue);
	}

	return runOptions;
}

static World* createEngineTestsWorld(
	Framework& framework,
	const string& worldName)
{
	return framework.createWorld(worldName);
}
	...
	const string& worldName)
{
	...
	return createEngineTestsWorld(framework, worldName) != nullptr;
}
...
static int32 runFrameworkTests(WindowsWindowObject& windowsWindowObject)
{
	...
	if (createEngineTestsWorld(framework, "FrameworkTestRuntime") == nullptr)
	{
		framework.shutdown();
	...
	{
		if (!ensureEngineTestsActiveWorld(framework, "FrameworkTestRuntime"))
		{
	...
	const EngineTestsRunOptions& runOptions)
{
	...
	if (createEngineTestsWorld(framework, "BackendTestRuntime") == nullptr)
	{
		framework.shutdown();
	...
	while (windowsWindowObject.pumpMessages())
	{
		if (!ensureEngineTestsActiveWorld(framework, "BackendTestRuntime"))
		{
...
static int32 runReplayTests(
	WindowsWindowObject& windowsWindowObject,
	const EngineTestsRunOptions& runOptions)
{
	FrameworkReplayTest::InitializeOptions frameworkInitializeOptions = {};
	frameworkInitializeOptions.backendType = runOptions.backendType;
	frameworkInitializeOptions.enableDebugLayer = runOptions.enableBackendDebugLayer;
	frameworkInitializeOptions.validationInjectMode = runOptions.backendValidationInjectMode;

	FrameworkReplayTest framework = {};
	if (!framework.initialize(windowsWindowObject, frameworkInitializeOptions))
	{
		const FrameworkRuntimeExitCode initializeExitCode = framework.getRuntimeExitCode();
		framework.shutdown();
	...
	EditorReplayRunner replayRunner = {};
	if (!replayRunner.load(runOptions.editorReplayFilePath))
	{
		renderWorld.shutdown();
		framework.shutdown();
		return static_cast<int32>(EngineTestsExecutionCode::replayLoadFailed);
	}

	EngineTestsExecutionCode executionCode = EngineTestsExecutionCode::success;
	while (windowsWindowObject.pumpMessages())
	{
		if (replayRunner.hasPendingCommands() && !replayRunner.step())
		{
			executionCode = EngineTestsExecutionCode::replayCommandFailed;
			break;
		}

		if (!framework.update())
		{
			error << "[EditorReplayTest] action=failed stage=framework_update" << lineBreak;
			executionCode = EngineTestsExecutionCode::testTickFailed;
			break;
		}
		...
		renderWorldUpdateInput.renderCommandFlushInput.clearOnly = false;
		renderWorldUpdateInput.renderCommandFlushInput.validateAfterFlush = true;
		renderWorldUpdateInput.renderCommandFlushInput.processBackendValidationFailFast = [&framework, &validationFailed]() -> bool
		{
			const bool failed = processEngineTestsBackendValidationFailFast(framework, "editor_replay");
			validationFailed = validationFailed || failed;
			return failed;
		};
		if (!renderWorld.update(renderWorldUpdateInput))
		{
			error << "[EditorReplayTest] action=failed stage=render_world_update" << lineBreak;
			executionCode = EngineTestsExecutionCode::renderWorldUpdateFailed;
			break;
		}
		...
		if (validationFailed)
		{
			executionCode = EngineTestsExecutionCode::replayValidationFailed;
			break;
		}

		if (replayRunner.isCompleted())
		{
			break;
		}
	...
	if (executionCode == EngineTestsExecutionCode::success && replayRunner.hasFailed())
	{
		executionCode = EngineTestsExecutionCode::replayCommandFailed;
	}

	if (executionCode == EngineTestsExecutionCode::success && !replayRunner.isCompleted())
	{
		error << "[EditorReplayTest] action=failed stage=window_loop_ended_before_completion" << lineBreak;
		executionCode = EngineTestsExecutionCode::replayCommandFailed;
	}
	...
	int commandShow)
{
	...
	assert(diskLoaderModule != nullptr && "[EngineTests][Assert] reason=disk_loader_module_missing");
	...
	const int32 executionCode =
		runOptions.runMode == EngineTestsRunMode::backendMode
			...
			: (runOptions.runMode == EngineTestsRunMode::replayMode
				? runReplayTests(windowsWindowObject, runOptions)
